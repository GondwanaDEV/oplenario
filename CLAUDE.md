...
**Estado MÉDIA (7/~13 feitas) + TODOS OS 10 ARQUÉTIPOS PROVADOS:** ✅ **Login/MFA** (`login.html`, arquétipo **auth**, passkey-primário) · ✅ **Central de notificações** (`notificacoes.html`, inbox) · ✅ **Cadastro de vereadores** (`cadastro-vereadores.html`, master-detail) · ✅ **Livro de atas** (`livro-atas.html`, papel+lista) · ✅ **Ouvidoria 13.460** (`ouvidoria.html`, pública+form) · ✅ **Telão de votação** (`telao-votacao.html`, arquétipo **display** — projeção single-mode escuro) · ✅ **Calendário institucional** (`calendario.html`, arquétipo **calendário**). Commits: `1b35ef8` `aa9c4b7` `8b43179` `23ced11` `f44ab29` `56c93d2` `ddf5d6b`. **Os 10 arquétipos do catálogo estão provados** (cabine·balcão·pública·cockpit·lista·ficha·wizard·config·display·calendário) — toda tela restante é **aplicação pura**.

**Primeira ação agora — FASE E (MÉDIA restantes = só reaplicação):** seguir pelas MÉDIA que faltam do `INVENTARIO-TELAS.md §2`, cada uma **aplicando** arquétipo já provado: **Admin do ente — usuários** (1.8, lista+config) · **Comissões/Mesa** (2.2/2.3, lista+ficha/config) · **Pareceres** (3.5, balcão) · **Pós-aprovação sanção→lei** (3.13-15, tracking/ficha) · **Portal — navegação/lista/ficha pública** (5.2, pública) · **Legislação consolidada** (5.8, pública/lista); depois BAIXA (~8). **Sem arquétipo novo a inventar.** **Cada tela (protocolo obrigatório):** linka `../sistema/`, escolhe arquétipo, reusa receitas, **roda o `GUIDELINES-CHECKLIST.md` como gate** (esp. §5.1 — armadilhas de contraste medidas: branco-sobre-telha→`--telha-fundo`; âmbar-sobre-surface→`--aviso-texto`; secundário-sobre-`papel-2` escurece no escuro), valida 2 temas + a11y AA **medida em pixel composto, um tema por chamada com flush**, commita (uma tela/commit). **Promover ao chassi as receitas ≥4 usos** (camada de IA, azulejo, ilha-papel, chips, campos, `.card`) — `PADROES §3/§5`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor:** `python3 -m http.server 8755` na raiz; galeria em `componentes.html`. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `ddf5d6b`; **21 telas feitas** = 4 herói + 8 ALTA + 7 MÉDIA + Eixo 4; os 10 arquétipos provados).
