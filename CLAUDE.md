...
**ESCALAR O DESIGN SYSTEM — em curso (mandato Emilio 22/06: fazer sozinho TODA a parte faltante do design, parar só quando não der p/ decidir).** Plano em 5 eixos: **0+1** Fundação extraída + Biblioteca viva · **3** Inventário de telas · **4** Guidelines-checklist · **2** 4 arquétipos faltantes (lista/tabela · ficha · wizard · config) · **E** telas reais priorizadas.
...
**Primeira ação agora — Eixo 3 (Inventário de telas):** cruzar as **113 features / 12 módulos** (`produto/13`) × 8 arquétipos × cobertura × prioridade num `INVENTARIO-TELAS.md`, depois seguir p/ Eixo 2 (arquétipos, começando por **lista/tabela filtrável**) e as telas reais ALTA-prioridade (editor de proposição 3.1/3.11 · tramitação 3.3/3.7/3.8 · pauta+convocação 4.14/4.15 · revisão ata-IA 4.13/8.5 · dashboard vereador PWA 7.2). Toolchain (§4): `frontend-design` + `ui-ux-pro-max`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) é input intacto. **Servidor de validação:** `python3 -m http.server 8755` na raiz (telas linkam `../sistema/`, galeria `sistema/`). **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (commit de DS: `4de6bd5`).
