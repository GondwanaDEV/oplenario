...
**Primeira ação recomendada agora — ESCALAR O DESIGN SYSTEM (próximo macro-passo, em sessão fresca dedicada):** as **4 telas-que-decidem-a-compra estão FEITAS** (sessão ao vivo · Expediente · Portal do Cidadão · Painéis da Mesa — os **3 públicos decisores cobertos**) e a linguagem **"República Luminosa"** (dual-theme claro/escuro) está provada. O próximo passo **deixa de ser telas-herói** e vira **escalar o design system ao catálogo amplo (113 features / 12 módulos)**: extrair dos 4 protótipos uma **biblioteca de componentes/padrões reusáveis** (os tokens semânticos por `[data-tema]` já existem — vivem em `LINGUAGEM-VISUAL.md` + `telas/*.html`) + um **inventário de telas por módulo** que priorize o que falta para fechar os fluxos (a decomposição das 113 features está em `produto/13`). Toolchain (§4): `frontend-design` (direção+copy) + `ui-ux-pro-max` (catálogo/UX/charts); artefatos à mão em `produto/design-system/`. **Fronteira do descarte continua valendo:** só `produto/design-system/` é design; o resto de `produto/` (01–14) é input intacto. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (chat de stack §22.4.4). **Branch atual: `design-do-zero`** (últimos commits de design: `e427ee5` tela Painéis da Mesa + `b2940f0` a11y AA + tidy).
