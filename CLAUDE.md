...
**ESCALAR O DESIGN SYSTEM — em curso (mandato Emilio 22/06: fazer sozinho TODA a parte faltante do design, parar só quando não der p/ decidir).** Plano em 5 eixos: **✅0+1** Fundação + Biblioteca viva · **✅3** Inventário · **✅2** 4 arquétipos novos · **4** Guidelines-checklist (pendente) · **E** telas reais (em curso). **O design system está TOTALMENTE ESCAFOLDADO: os 8 arquétipos de página estão provados** (cabine·balcão·pública·cockpit + lista·ficha·wizard·config). **8 telas-herói feitas** (Sessão ao vivo · Expediente · Portal · Painéis · Proposições · Ficha da matéria · Nova proposição · Config da Câmara). Commits desta sessão: `4de6bd5`(0+1) `0e47558` `33d561a`(inventário) `1fa54f4`(lista) `d5a10b7`(ficha) `6fac030`(wizard) `71c330a`(config).
...
**Primeira ação agora — FASE E (telas reais, aplicam arquétipos provados):** desenhar as telas ALTA-prioridade que faltam, na sequência do `INVENTARIO-TELAS.md §2`: **editor de proposição + copiloto** (3.11, balcão+IA) → **tramitação board/kanban** (3.3/3.7/11.3, cockpit) → **minhas pendências** (11.2, cockpit/lista) → **pauta + convocação** (4.1/4.14/4.15, balcão/wizard) → **revisão de ata-IA** (4.13/8.5, balcão+IA) → **dashboard vereador PWA** (7.2, cockpit mobile); depois MÉDIA (login/MFA, admin usuários, cadastros, livro de atas, central de notificações, ouvidoria, navegação pública…). **Cada tela:** linka `../sistema/`, escolhe o arquétipo, reusa receitas (azulejo/ilhas/chips/IA-trust), valida 2 temas + a11y AA medida, commita. **Eixo 4 (guidelines-checklist):** formalizar voz/a11y/anti-IA/[GAP] como gate — pode fechar junto ou ao final. **Padrão de qualidade:** revisão adversarial (workflow) nas telas-herói novas; promover receitas ao chassi no 2º uso (PADROES §1). **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor de validação:** `python3 -m http.server 8755` na raiz (telas linkam `../sistema/`, galeria `sistema/`). **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `71c330a`).
