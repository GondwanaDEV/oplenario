...
**Primeira ação agora — FASE E (MÉDIA), aplicar arquétipos provados ao resto do catálogo:** as ALTA acabaram; seguir pelas telas **MÉDIA** do `INVENTARIO-TELAS.md §2`, cada uma **aplicando** (não inventando) um arquétipo já provado: **Login/MFA** (1.1, config/auth) · **Admin do ente — usuários** (1.8, lista+config) · **Cadastro de vereadores** (2.1, lista+ficha) · **Comissões/Mesa** (2.2/2.3) · **Calendário institucional** (2.6, calendário — arquétipo novo leve) · **Livro de atas** (4.21, lista+ficha) · **Central de notificações** (11.6, lista/inbox) · **Ouvidoria** (6.2, pública+form) · **Portal — navegação/lista/ficha pública** (5.2) · **Legislação consolidada** (5.8) · **Pareceres** (3.5, balcão) · **Pós-aprovação sanção→lei** (3.13-15, tracking) · **Telão de votação** (4.2, display). **Cada tela (protocolo obrigatório):** linka `../sistema/`, escolhe arquétipo, reusa receitas, **roda o `GUIDELINES-CHECKLIST.md` como gate** (esp. §5.1 — armadilhas de contraste medidas: branco-sobre-telha→`--telha-fundo`; âmbar-sobre-surface→`--aviso-texto`; secundário-sobre-`papel-2` escurece no escuro), valida 2 temas + a11y AA **medida em pixel composto, um tema por chamada com flush**, commita (uma tela/commit). **Promover ao chassi as receitas ≥4 usos** (camada de IA, azulejo, ilha-papel, chips, campos, `.card`) — `PADROES §3/§5`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor:** `python3 -m http.server 8755` na raiz; galeria em `componentes.html`. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `2c41a28`).
