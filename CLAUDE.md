...
**Estado MÉDIA (5/~13 feitas):** ✅ **Login/MFA** (`login.html`, arquétipo auth novo, passkey-primário) · ✅ **Central de notificações** (`notificacoes.html`, inbox) · ✅ **Cadastro de vereadores** (`cadastro-vereadores.html`, master-detail) · ✅ **Livro de atas** (`livro-atas.html`, papel+lista) · ✅ **Ouvidoria 13.460** (`ouvidoria.html`, pública+form). Commits: `1b35ef8` `aa9c4b7` `8b43179` `23ced11` `f44ab29`.

**Primeira ação agora — FASE E (MÉDIA restantes), aplicar arquétipos provados:** seguir pelas MÉDIA que faltam do `INVENTARIO-TELAS.md §2`, cada uma **aplicando** (não inventando) arquétipo provado: **Admin do ente — usuários** (1.8, lista+config) · **Comissões/Mesa** (2.2/2.3, lista+ficha/config) · **Calendário institucional** (2.6, arquétipo calendário novo leve) · **Telão de votação** (4.2, arquétipo display novo — o telão do plenário) · **Pareceres** (3.5, balcão) · **Pós-aprovação sanção→lei** (3.13-15, tracking) · **Portal — navegação/lista/ficha pública** (5.2) · **Legislação consolidada** (5.8); depois BAIXA (~8). Os 2 arquétipos ainda não provados = **calendário** e **display** (telão). **Cada tela (protocolo obrigatório):** linka `../sistema/`, escolhe arquétipo, reusa receitas, **roda o `GUIDELINES-CHECKLIST.md` como gate** (esp. §5.1 — armadilhas de contraste medidas: branco-sobre-telha→`--telha-fundo`; âmbar-sobre-surface→`--aviso-texto`; secundário-sobre-`papel-2` escurece no escuro), valida 2 temas + a11y AA **medida em pixel composto, um tema por chamada com flush**, commita (uma tela/commit). **Promover ao chassi as receitas ≥4 usos** (camada de IA, azulejo, ilha-papel, chips, campos, `.card`) — `PADROES §3/§5`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor:** `python3 -m http.server 8755` na raiz; galeria em `componentes.html`. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `f44ab29`; 19 telas feitas: 4 herói + 4 arquétipos novos + 8 ALTA-restantes-da-Fase-E... na verdade 8 ALTA + 5 MÉDIA + Eixo 4).
