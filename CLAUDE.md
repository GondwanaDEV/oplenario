...
**Primeira ação agora — só a tier BAIXA (diferível/nicho/operador), reaplicação pura:** se/quando retomar, seguir o `INVENTARIO-TELAS.md §2` BAIXA: **Audiência pública** (4.18) · **Julgamento de contas** (4.19) · **Transparência fiscal** (5.9) · **Dados abertos** (5.11) · **Status page** (10.3) · **Exportar dados do ente** (9.6) · **Console do operador SaaS** (12.x) · **cadastros menores** (2.4/2.5). Cada uma **aplica** arquétipo já provado e roda o `GUIDELINES-CHECKLIST` (esp. §5.1). **Nenhum arquétipo novo; nenhuma decisão de design em aberto.** O catálogo que decide a compra e a paridade institucional está 100% desenhado. **Cada tela (protocolo obrigatório):** linka `../sistema/`, escolhe arquétipo, reusa receitas, **roda o `GUIDELINES-CHECKLIST.md` como gate** (esp. §5.1 — armadilhas de contraste medidas: branco-sobre-telha→`--telha-fundo`; âmbar-sobre-surface→`--aviso-texto`; secundário-sobre-`papel-2` escurece no escuro), valida 2 temas + a11y AA **medida em pixel composto, um tema por chamada com flush**, commita (uma tela/commit). **Promover ao chassi as receitas ≥4 usos** (camada de IA, azulejo, ilha-papel, chips, campos, `.card`) — `PADROES §3/§5`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor:** `python3 -m http.server 8755` na raiz; galeria em `componentes.html`. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `79df7ea`; **27 telas feitas** = 4 herói + 8 ALTA + 13 MÉDIA + Eixo 4; os 10 arquétipos provados; resta só BAIXA, diferível).
