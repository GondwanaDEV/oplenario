...
**✅ BAIXA FECHADA (8/8) — CATÁLOGO DE DESIGN 100% COMPLETO: 35 telas.** BAIXA feita: Audiência pública (`audiencia-publica.html`) · Julgamento de contas (`julgamento-contas.html`) · Transparência fiscal (`transparencia-fiscal.html`, charts honestos) · Dados abertos (`dados-abertos.html`) · Status page (`status.html`) · Exportar dados do ente (`exportar-dados.html`, sem lock-in) · Console do operador SaaS (`console-operador.html`, supratenant) · Cadastros menores (`cadastros-menores.html`). Commits BAIXA: `d03316a`·`f3ca46c`·`f3f253d`·`84b7724`·`06ee89a`·`bf6e0db`·`9f6a6d3`·`29debee`.

**⚠️ Auditoria de completude (22/06) — o "design 100%" NÃO se sustentou.** Passe independente (subagente) cruzou as 113 features × as 35 telas (abrindo o HTML) e achou **13 GAPS** + ~13 coberturas fracas; verificados. Cobertura real ~90%, não 100%. Detalhe em `INVENTARIO-TELAS.md §4`. Os GAPS que importam (tenant-facing): **(1) trilha de auditoria do tenant** (1.6) · **(2) camada pública navegável** — ficha pública da matéria + perfil do vereador + comentários/moderação (5.2/6.3) · **(3) lado do vereador** — coautoria (3.17), assinatura 2-toques (7.3), estatísticas (7.5) [enfraquecem a Aposta 2] · (4) anexar ata externa (4.11), closed-caption (4.20), SSO gov.br (1.2); + 4 do console operador (12.3/12.5/12.7/8.6, BAIXA). `governanca-ia-clj/` JÁ está versionado (cursor antigo dizia untracked — corrigido).

**Primeira ação agora — fechar os GAPS de design antes de partir p/ engenharia.** Ordem sugerida: (a) **trilha de auditoria** (1 tela, arquétipo lista) — jurídico/Invariante 10; (b) **ficha pública da matéria + perfil público do vereador + comentários/moderação** (~3 telas, arquétipo pública) — fecha a transparência navegável; (c) **lado do vereador** (coautoria no wizard + assinatura 2-toques + estatísticas, ~2-3 telas/incrementos) — fecha a Aposta 2; depois os menores. Cada uma reusa arquétipo provado + roda o `GUIDELINES-CHECKLIST`. **Só então** o design fecha de verdade e segue a engenharia (materializar front real §22.9 + motor §22.4.4). **Cada tela (protocolo obrigatório):** linka `../sistema/`, escolhe arquétipo, reusa receitas, **roda o `GUIDELINES-CHECKLIST.md` como gate** (esp. §5.1 — armadilhas de contraste medidas: branco-sobre-telha→`--telha-fundo`; âmbar-sobre-surface→`--aviso-texto`; secundário-sobre-`papel-2` escurece no escuro), valida 2 temas + a11y AA **medida em pixel composto, um tema por chamada com flush**, commita (uma tela/commit). **Promover ao chassi as receitas ≥4 usos** (camada de IA, azulejo, ilha-papel, chips, campos, `.card`) — `PADROES §3/§5`. **Fronteira do descarte:** só `produto/design-system/` é design; resto de `produto/` (01–14) intacto. **Servidor:** `python3 -m http.server 8755` na raiz; galeria em `componentes.html`. **Pendências paralelas (não bloqueiam):** commitar `governanca-ia-clj/` (untracked); materialização do motor (§22.4.4). **Branch: `design-do-zero`** (último commit de DS: `29debee`; **35 telas = catálogo de design COMPLETO** = 4 herói + 8 ALTA + 13 MÉDIA + 8 BAIXA + 2 arquétipos-base (lista/ficha/wizard/config contam nas ALTA) + Eixo 4; os 10 arquétipos provados; **nada de design resta**).
