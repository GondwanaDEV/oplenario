(ns oplenario.sessoes.chamada-conduzida-http-in-test
  ...
  seam `roster-da-casa` (fake aqui); `ocorrido-em` vem do RELOGIO real do host (esta rota nao aceita relogio
...
(deftest conduzir-chamada-201
  (let [ente (random-uuid) sid (random-uuid) op (random-uuid)
    ...
    (is (= 2 (:membros-da-casa @cap)) "membros-da-casa computado do roster (2 vereadores) — NAO vem do corpo")
...
(deftest conduzir-chamada-sem-membros-no-roster-e-201-com-denominador-zero
  (let [ente (random-uuid) op (random-uuid)
    ...
    (is (= 201 (:status r)))
    (is (zero? (:membros-da-casa @cap)) "roster vazio -> denominador 0 (nao e' erro, e' um fato do cadastro)")))
