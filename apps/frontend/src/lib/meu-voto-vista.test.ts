...
describe("derivarMeuVoto", () => {
  ...
  it("votação nominal aberta, presente, sem voto ainda → pode-votar", () => {
    const estado: EstadoPlenario = {
      ...
      presentes: ["v1"],
      placar: {
        votacaoId: "vot1", modalidade: "nominal", objetoTipo: "proposicao", objetoId: "p1", proposicao: null, encerrada: false,
  ...
  it("modalidade secreta (ou não provada) → nunca oferece o botão", () => {
    const estado: EstadoPlenario = {
      ...BASE,
      placar: {
        votacaoId: "vot1", modalidade: "secreta", objetoTipo: "proposicao", objetoId: "p1", proposicao: null, encerrada: false,
  ...
  it("secreta E encerrada → 'encerrada' vence (a checagem de estado roda ANTES da de modalidade)", () => {
    ...
    const estado: EstadoPlenario = {
      ...BASE,
      placar: {
        votacaoId: "vot1", modalidade: "secreta", objetoTipo: "proposicao", objetoId: "p1", proposicao: null, encerrada: true,
  ...
  it("votação encerrada → encerrada (mesmo se antes desse voto)", () => {
    const estado: EstadoPlenario = {
      ...BASE,
      placar: {
        votacaoId: "vot1", modalidade: "nominal", objetoTipo: "proposicao", objetoId: "p1", proposicao: null, encerrada: true,
