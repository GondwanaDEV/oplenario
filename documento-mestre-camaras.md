...
camada `tce_estadual`), não antes (disc. 6 — generalizar no 2º caso); a *forma* do port é barata agora,
...
**Fecha o terceiro dos +5; resta +1 — expansão a outros TCEs** (S2, `dominio` em camadas; o port
...
- **Síncrono → HTTP:** o `controller` depende de um `port` (protocolo); a impl é um **HTTP client** (Component) que chama o `http_server` do outro módulo. URL local agora, remoto após extração — config, não código. O `port` deixa o controller testável com fake.
