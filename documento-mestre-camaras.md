...
**Enforcement:** import-lint (clj-kondo) sobre a matriz de dependência — um módulo nunca importa namespace de outro módulo (só HTTP/evento); `kernel`/`motor` nunca importam um módulo; `db`/`logic`/`internal` de um ctx só são importados por ele. Soma-se o teste de vazamento cross-tenant com dois entes sintéticos (§22.2) e a cobertura de autorização (§22.5 eixo E). CI falha o build em violação.
